...
class donatorAgreementController @Inject()(appConfig: FrontendAppConfig,
  ...
  def onPageLoad(mode: Mode) = (getData andThen requireData) {
    implicit request =>
      ...
      Ok(donatorAgreement(appConfig, preparedForm, mode))
  }
    ...
    implicit request =>
      form.bindFromRequest().fold(
        (formWithErrors: Form[_]) =>
          Future.successful(BadRequest(donatorAgreement(appConfig, formWithErrors, mode))),
        (value) =>
